...
# Arguments
# 
# fpath        Character. Path to the directory with all the csv output
#              files with suffix "all_mean_est.csv".
# name_dirs    Character, length `n`. As many entries as datasets. Same
#              names that you use as preffix to generate the output csv files
#              with suffix "all_mean_est.csv".
divt_csv <- function( fpath, name_dirs )
{
  # Create objects for each dataset that will be required for plotting
  GBM_all <- ILN_all <- vector( mode = "list", length = length( name_dirs ) )
  cat( "Parsing datasets ...\n" )
  cat( "[[ PATH: ", fpath, "]]\n" )
  all_csv  <- list.files( fpath, pattern = "all_mean_est.tsv" )
  ind_GBM  <- grep( pattern = "GBM", x = all_csv )
  GBM_csv  <- all_csv[ind_GBM]
  ind_GBM_filt <- grep( pattern = "FILT", x = GBM_csv )
  ##> Keep only FILT files if available
  if( length( ind_GBM_filt ) > 0 ){
    GBM_filt_name <- gsub( pattern = "_FILT", x = GBM_csv[ind_GBM_filt],
                           replacement = "" )
    rm_GBM_unfilt <- which( GBM_csv %in% GBM_filt_name )
    GBM_all_csv   <- GBM_csv[-rm_GBM_unfilt]
  }else{
    GBM_all_csv   <- GBM_csv
  }
  ##> Done!
  # Read GBM files
  # File names ordered according to "sort"
  for( i in 1:length( GBM_all_csv ) ){
    cat( " Parsing dataset:", GBM_all_csv[i], "\n" )
    is_filt <- grep( pattern = "FILT", x = GBM_all_csv[i] )
    if( length( is_filt ) > 0 ){
      cat( "   ---> This is a filtered dataset!\n" )
    }
    GBM_all[[ i ]] <- read.table( file = paste( fpath, GBM_all_csv[i],
                                                sep = "" ), header = TRUE,
                                  stringsAsFactors = FALSE, sep = "\t" )
    names( GBM_all )[i] <- gsub( x = GBM_all_csv[i],
                                 pattern = "_all_mean_est.tsv",
                                 replacement = "" )
  }
  # Get the rest of GBM files if there were any FILT files
  ...
  if( length( ind_ILN_filt ) > 0 ){
    ILN_filt_name <- gsub( pattern = "_FILT", x = ILN_csv[ind_ILN_filt],
                           replacement = "" )
    rm_ILN_unfilt <- which( ILN_csv %in% ILN_filt_name )
    ILN_all_csv   <- ILN_csv[-rm_ILN_unfilt]
  }else{
    ILN_all_csv   <- ILN_csv
  }
  ##> Done!
  # Read ILN files
  for( i in 1:length( ILN_all_csv ) ){
    cat( " Parsing dataset:", ILN_all_csv[i], "\n" )
    is_filt <- grep( pattern = "FILT", x = ILN_all_csv[i] )
    if( length( is_filt ) > 0 ){
      cat( "   ---> This is a filtered dataset!\n" )
    }
    ILN_all[[ i ]] <- read.table( file = paste( fpath, ILN_all_csv[i],
                                                sep = "" ), header = TRUE,
                                  stringsAsFactors = FALSE, sep = "\t" )
    names( ILN_all )[i] <- gsub( x = ILN_all_csv[i],
                                 pattern = "_all_mean_est.tsv",
                                 replacement = "" )
  }
                              ...
                              suffix = "", pch_vals, cex_vals )
{
  ...
  for( k in 1:length( plots_per_doc ) ){
    ...
    for( i in tmp_ind_lab ){
      ...
      # Get node label
      node_label <- gsub( x = rownames( sum_obj$GBM[[ j ]] )[i],
                          pattern = "t_n[0-9]*_", replacement = "" )
      ...
      plot( x = x_vals, 
            ...
            ylim = c( min_y, max_y ),
            xaxt = "n", xlab = "",
            ylab = "Estimated mean divergence times (Ma)",
            col = points_col )
      text( x = x_vals+c( sep_space ), # offset calculated by eye
            y =  c( y_meant_GBM, y_meant_ILN )+c( sep_space ),
            labels =  c( y_meant_GBM, y_meant_ILN ),
            srt = 360, adj = 1, xpd = TRUE )
      ...
      points( x = x_vals, y = c( y_CImax_GBM, y_CImax_ILN ), pch = 25,
              ...
              col = points_col )
      text( x = x_vals+c( sep_space ), # offset calculated by eye
            y = c( y_CImax_GBM, y_CImax_ILN )+c( sep_space ),
            labels = c( y_CImax_GBM, y_CImax_ILN ),
            srt = 360, adj = 1, xpd = TRUE )
      ...
      points( x = x_vals, y = c( y_CImin_GBM, y_CImin_ILN ), pch = 2,
              ...
              col = points_col )
      text( x = x_vals+c( sep_space ), # offset calculated by eye
            y = c( y_CImin_GBM, y_CImin_ILN ),
            labels = c( y_CImin_GBM, y_CImin_ILN ),
            srt = 360, adj = 1, xpd = TRUE )
    }
    # Close graphics
    dev.off()
...
#                use different colours from black to identify them in the
#                plots generated for each mirrored node.
comparison_plots_add <- function( sum_obj, out_dir, plots_per_doc, all_nodes,
                                  lab_nodes,
                                  data_perclock = 4, x_labs,
                                  ...
                                  suffix = "", pch_vals, cex_vals )
{
  ...
  for( k in 1:length( plots_per_doc ) ){
    ...
    ##> END
    plot_ns <- all_nodes[start:end]
    lab_nodes <- gsub( x = rownames( sum_obj$GBM[[ k ]] ),
