...
# Get args
aamod=$1 # Type of `model` selected to run CODEML
dir=$2   # Number to label the directory to be created
...
then
# enable `model = 2`
sed -i 's/AAMOD/2/' $codeml_dir/*ctl
elif [[ $aamod -eq 2 ]]
then
# enable `model = 3`
sed -i 's/AAMOD/3/' $codeml_dir/*ctl
fi
