...
pipeline_dir=$( pwd )
main_dir=$( echo $pipeline_dir | sed 's/\/modsel_animals_b128\/..*/\/modsel_animals_b128\//' )
cd $main_dir/Hessian
baseml_dir=$( pwd )
name_inBV=$( echo in.BV )
path_inBV=$( echo $baseml_dir/$name_inBV )
cd $main_dir/MCMCtree/dRetal15pUhb_ILN/$SGE_TASK_ID/
home_dir=$( pwd )
cd $main_dir/alignments/
aln_dir=$( pwd )
name_aln=`ls *txt`
path_aln=$( echo $aln_dir/$name_aln | sed 's/\_/\\\_/g' | sed 's/\//\\\//g' | sed 's/\./\\\./g' )
cd $main_dir/trees/calibrated/dRetal15pUhb_ILN
tree_dir=$( pwd )
name_tree=`ls *tree`
path_tree=$( echo $tree_dir/$name_tree | sed 's/\_/\\\_/g' | sed 's/\//\\\//g' | sed 's/\./\\\./g' )

# 3 Create specific log file
exec 3>&1> >(while read line; do echo "$line" >> $pipeline_dir/log.MCMCtree_b$SGE_TASK_ID".txt"; done;) 2>&1
start=`date`
