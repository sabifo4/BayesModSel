...
dat <- c( "Unconstrained-GBM", "Unconstrained-ILN",
...
delcol_post <- 4 # There are 2 partitions, and hence 4 columns (we ran
...
# of chains we ran).
all_paths <- c( paste( home_dir,
                       "../00_MCMCtree/sum_analyses/01_posterior/dRetal15pUhb_GBM/",
                       sep = "" ),
                paste( home_dir,
                       "../00_MCMCtree/sum_analyses/01_posterior/dRetal15pUhb_ILN/",
                       sep = "" ),
                paste( home_dir,
                       "../00_MCMCtree/sum_analyses/01_posterior/BM23pUhb_GBM/",
                       sep = "" ),
                paste( home_dir,
                       "../00_MCMCtree/sum_analyses/01_posterior/BM23pUhb_ILN/",
                       sep = "" )
)
#--------------#
# ANALYSE DATA #
...
out_dat       <- all_paths
time_unit     <- 100
out_file_pdf  <- paste( "Convergence_plot_post_", dat, sep = "" )
out_title_pdf <- paste( "post_", dat, sep = "" )
th            <- c( 0.1, 0.1,  # dRetal15pUhb
                    0.2, 0.1 ) # BM23pUhb -- harder MCMC space
...
# Save object! -- it is a very big file (~2Gb), so only run the commands below
...
## [[ Unconstrained-GBM ]] ####
sum_post_QC$`Unconstrained-GBM`$ESS_results
# $median
# [1] 20001
# 
# $min
...
# $total_samples
# [1] 60003    53
length( sum_post_QC$`Unconstrained-GBM`$not_conv_nodes ) # 0
sum_post_QC$`Unconstrained-GBM`$num_chains # 3


## [[ Unconstrained-ILN ]] ####
sum_post_QC$`Unconstrained-ILN`$ESS_results
# $median
# [1] 20001
# 
# $min
...
# $total_samples
# [1] 120006     53
length( sum_post_QC$`Unconstrained-ILN`$not_conv_nodes ) # 0
sum_post_QC$`Unconstrained-ILN`$num_chains # 6

## [[ UpperEdiacaran-GBM ]] ####
sum_post_QC$`UpperEdiacaran-GBM`$ESS_results
# $median
# [1] 20001
# 
# $min
...
# $total_samples
# [1] 80004    53
length( sum_post_QC$`UpperEdiacaran-GBM`$not_conv_nodes ) # 0
sum_post_QC$`UpperEdiacaran-GBM`$num_chains # 4

## [[ UpperEdiacaran-ILN ]] ####
sum_post_QC$`UpperEdiacaran-ILN`$ESS_results
# $median
# [1] 20001
# 
# $min
...
# $total_samples
# [1] 100005     53
length( sum_post_QC$`UpperEdiacaran-ILN`$not_conv_nodes ) # 0
sum_post_QC$`UpperEdiacaran-ILN`$num_chains # 5
