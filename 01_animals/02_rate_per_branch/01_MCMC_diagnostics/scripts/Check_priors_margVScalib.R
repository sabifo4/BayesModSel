...
#
# dRetal15: The number of lines is 80004 (NODAT-GBM) and 100005 (NODAT-ILN);
#           you need to specify one less
# dRetal15pUhb: The number of lines is 80004 (NODAT-GBM) and 100005 (NODAT-ILN);
#               you need to specify one less
# BM23: The number of lines is 80004 for both NODAT-GBM NODAT-ILN; you need to
#       specify one less
# BM23pUhb: The number of lines is 40002 (NODAT-GBM) and 120006 (NODAT-ILN);
#           you need to specify one less
#
# NOTE: If you had more than one dataset, you can add another number to
# the `def_samples` vector if they were different. If they are the same,
# you can just keep one
# E.g.: two datasets with same size: c( 100005, 100005 ) OR c( 100005 )
# E.g.: two datasets with diff size: c( 80003, 100005 )
def_samples <- c( c( 80003, 100004 ), # dRetal15pUhb | NDGBM, NDILN
                  c( 40001, 120005 )  # BM23pUhb | NDGBM, NDILN
)
...
delcol_prior <- 4 # There are 2 partitions, and hence 4 columns (we ran
...
for( i in 1:num_dirs ){
  ...
  plot_check_calibnodes( calibs = calib_nodes[[ count ]],
                         divt_list = mcmc_priors[[ i ]], 
                         dat = dat[count], out = names( mcmc_priors )[count],
                         # Adding dataset name + "NODAT"; variable
                         # `clock` adds a suffix to output file with all plots!
                         clock = names( mcmc_priors )[count],
                         main_wd = home_dir, ind = TRUE,
