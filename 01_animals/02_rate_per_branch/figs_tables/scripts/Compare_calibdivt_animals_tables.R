...
dir_names <- sort( c( "Unconstrained","UpperEdiacaran" ) )
all_divt  <- divt_csv( fpath = divcsv_wd, name_dirs = dir_names )
# Select nodes that are to be plotted:
nodes_calibs <- grep( x = rownames( all_divt$GBM$`Unconstrained-GBM_FILT` ),
                      pattern = "t_n[0-9]*_" )
nodes_2plot <- gsub( x = rownames( all_divt$GBM$`Unconstrained-GBM_FILT` )[ nodes_calibs ],
                     pattern = "_[A-Z].*", replacement = "" )
plots_per_fig <- rep( 1, length( nodes_2plot ) )
names( nodes_2plot ) <- gsub( x = rownames( all_divt$GBM$`Unconstrained-GBM_FILT` )[ nodes_calibs ],
                              pattern = "t_n", replacement = "tn" )
# Get indexes
only_nums <- gsub( x = rownames( all_divt$GBM$`Unconstrained-GBM_FILT` ),
                   pattern = "_[A-Z]..*", replacement = "" )
...
# MCMC diagnostics)
cols_per_dataset <- length( colnames( all_divt$GBM$`Unconstrained-GBM_FILT` ) )-1
...
write.table( x = short_GBM, file = paste( main_wd, "figs_tables/tables/",
                                          "compare_calibnodes_divtimes_animals_GBM.tsv",
                                          sep = "" ),
             sep = "\t", quote = FALSE, row.names = TRUE, col.names = TRUE )
write.table( x = short_ILN, file = paste( main_wd, "figs_tables/tables/",
                                          "compare_calibnodes_divtimes_animals_ILN.tsv",
                                          sep = "" ),
             sep = "\t", quote = FALSE, row.names = TRUE, col.names = TRUE )
...
# `Compare_calibdivt_mammals_tables.R`, then you do not need to run it again
# in this script
