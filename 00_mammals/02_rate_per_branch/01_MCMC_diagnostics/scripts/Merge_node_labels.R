...
dat <- c( "Unconstrained", "PostKPg" )
# All node trees are the same, hence we will just get the first one!
node_tt <- list.files( path = "../../00_MCMCtree/sum_analyses/00_prior/",
                       pattern = "tree" )[1]
...
for( i in 1:length( dat ) ){
  ...
  # Unused for this dataset!
  # if( length( dat ) > 1 ){
  #   tt_list[[ i ]][[ 1 ]] <- ape::read.tree( file = paste( "../../00_MCMCtree/sum_analyses/00_prior/",
  #                                                          "node_tree_", i, ".tree",
  #                                                          sep = "" ) )  
  # }else{
  #   tt_list[[ i ]][[ 1 ]] <- ape::read.tree( file = paste( "../../00_MCMCtree/sum_analyses/00_prior/",
  ...
  tt_list[[ i ]][[ 2 ]] <- ape::read.tree( file = paste( "../calibs/raw/cal_data/",
                                                         dat[i],
...
if( ! dir.exists( "../calibs/inp_calibs" ) ){
  dir.create( "../calibs/inp_calibs" ) 
}
for( i in 1:length(dat) ){
  write( x = "Calib;node;Prior",
         file = paste( "../calibs/inp_calibs/Calibnodes_",
                       dat[i], ".csv", sep = "" ) )
  write( x = "Calib;node;Prior",
         file = paste( "../calibs/inp_calibs/Calibnodes_",
...
for( j in 1:length(dat) ){
  ...
  if( count_margVScalib == 0 ){
    unlink( x = paste( "../calibs/inp_calibs/Calibnodes_",
                       dat[j], "_margVScalib.csv", sep = "" ) )
  }
