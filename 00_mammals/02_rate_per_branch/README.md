...
* If you **are not** a Mac user (or you are a Mac user and have no problems with using the Linux command `sed`), please run the code snippet below:
    ...
    ```sh
    #------------------------------------#
    # RUN ONLY IF YOU ARE NOT A MAC USER #
    #------------------------------------#
    ...
    for i in `seq 1 6`
    do
    mkdir -p HPC/MCMCtree/{ACetal22_GBM/$i,ACetal22_ILN/$i,BM23pUhb_GBM/$i,BM23pUhb_ILN/$i}
    done
    cd HPC
    # Copy alignment
    mkdir aln
    ...
    for i in ACetal22_GBM ACetal22_ILN BM23pUhb_GBM BM23pUhb_ILN
    do
    printf "Copying control file for analysis "$i" ...\n"
    for j in `seq 1 6`
    do
    cp ../../../01_modsel_workflow/00_mcmc3r/MCMCtree/$i/1/*ctl MCMCtree/$i/$j/mcmctree_$i.ctl
    ...
    cp -R MCMCtree/ACetal22_GBM MCMCtree/ACetal22_NODAT_GBM
    cp -R MCMCtree/ACetal22_ILN MCMCtree/ACetal22_NODAT_ILN
    cp -R MCMCtree/BM23pUhb_GBM MCMCtree/BM23pUhb_NODAT_GBM
    cp -R MCMCtree/BM23pUhb_ILN MCMCtree/BM23pUhb_NODAT_ILN
    for i in ACetal22_NODAT_GBM ACetal22_NODAT_ILN BM23pUhb_NODAT_GBM BM23pUhb_NODAT_ILN
    do
...
for i in ACetal22_GBM ACetal22_NODAT_GBM ACetal22_NODAT_ILN ACetal22_ILN BM23pUhb_ILN BM23pUhb_GBM BM23pUhb_NODAT_GBM BM23pUhb_NODAT_ILN
do
...
# Make sure that you have created a directory called `brate_mammals`
# in your HPC scratch directory or similar so that these files can
# be successfully copied!
rsync -avz --copy-links HPC/* <uname>@<server>:<path>/brate_mammals/
```
...
```sh
# Run from `brate_mammals`, the main directory, once you are in the HPC so that the 
# paths are already formatted
home_dir=$( pwd )
cd scripts
chmod 775 *sh
for i in ACetal22_GBM ACetal22_NODAT_GBM ACetal22_NODAT_ILN ACetal22_ILN BM23pUhb_GBM BM23pUhb_ILN BM23pUhb_NODAT_GBM BM23pUhb_NODAT_ILN
do
...
# arg4   Name of the working directory, e.g., `brate_mammals` in the HPC
# arg5   Type of hypothesis being tested (i.e., [ACetal22|BM23|BM23pUhb]_[GBM|ILN|NODAT_GBM|NODAT_ILN] )
./generate_job_MCMCtree.sh $home_dir/pipelines_MCMCtree mcmctree 6 brate_mammals $i
done
...
##>      rsync -avz --copy-links <uname>@<server>:<path>/brate_mammals/pipelines_MCMCtree .
```
...
# `pipelines_MCMCtree/[ACetal22|BM23pUhb]_NODAT_[GBM|ILN]` 
# in your HPC 
# Please change directories until you are there,
# then run the following commands
# E.g.: if you were inside `ACetal22_NODAT_GBM`...
cd ACetal22_NODAT_GBM
chmod 775 *sh
qsub pipeline_ACetal22_NODAT_GBM.sh
...
```sh
# Run everything from `brate_mammals` in your HPC
num_chains=6
mkdir -p tmp_to_transfer/00_prior
cd tmp_to_transfer
for j in ACetal22_NODAT_GBM ACetal22_NODAT_ILN BM23pUhb_NODAT_GBM BM23pUhb_NODAT_ILN
do
...
rsync -avz --copy-links <uname>@<logdetails>:<path>/brate_mammals/tmp_to_transfer/00_prior .
# You can transfer directory `pipelines_MCMCtree` now or wait until
# you run `MCMCtree` when sampling from the posterior!
```

> [!NOTE]
> If you want to download the output files we generated, please download `02_mammals_divtimes_prior.zip` from [our FigShare repository](https://doi.org/10.6084/m9.figshare.32033958) and save directory `pipelines_MCMCtree` inside `00_mammals/02_rate_per_branch/00_MCMCtree/sum_analyses`. Then, decompress, `00_prior.zip` and save `00_prior` inside the same directory. Please note that this directory also includes the output files we generate during some of the MCMC diagnostics (e.g., directories `[BM23pUhb|ACetal22]_NODAT_[GBM|ILN]_*NODAT*/[PostKPg|Unconstrained]*` and `mcmc_files_*NODAT*`). If you want to run the subsequent steps, you can move these output files to another location and only keep the four directories with the 6 independent chains (i.e., `[BM23pUhb|ACetal22]_NODAT_[GBM|ILN]/[1-6]`).
...
Once the calibration files are ready, we can run the `R` script [`MCMC_diagnostics_prior.R`](01_MCMC_diagnostics/scripts/Merge_node_labels.R) and follow the detailed step-by-step instructions written in the scripts. In a nutshell, the protocol to follow will be the following:
...
5. Generate new convergence plots with those chains that have passed the filters.
...
name_dat=( 'ACetal22_NODAT_GBM' 'ACetal22_NODAT_ILN' 'BM23pUhb_NODAT_GBM' 'BM23pUhb_NODAT_ILN' )
num_dat=4
count=-1
for i in `seq 1 $num_dat`
do
count=$(( count + 1 ))
./Combine_MCMC.sh ${name_dat[count]} mcmc_files_${name_dat[count]} "`seq 1 6`" NODAT 20000 Y ${name_dat[count]}
done
```
...
> [!NOTE]
> Please note that we have kept directory `sum_files_prior` in this repository (see code snippet below). Nevertheless, if you want to also download the `plots` directory (this is generated and populated during MCMC diagnostics), please download `03_mammals_MCMCdiagnostics.zip` from [our FigShare repository](https://doi.org/10.6084/m9.figshare.32033958). Then, decompress this directory and `plots.zip` and save directory `plots` inside `00_mammals/02_rate_per_branch/01_MCMC_diagnostics`. Without the `plots` directory, the commands below will not work.
...
```sh
# Run from directories
# `pipelines_MCMCtree/[ACetal22|BM23pUhb]_[GBM|ILN]` 
# on your HPC 
# Please change directories until you are there,
# then run the following commands
# E.g.: if you were inside `ACetal22_GBM`...
cd ACetal22_GBM
chmod 775 *sh
qsub pipeline_ACetal22_GBM.sh
# Now, just change directories until you have visited
# all directories in which `MCMCtree` was run
# to sample from the posterior:
#   'ACetal22_GBM' 'ACetal22_ILN' 'BM23pUhb_GBM' 'BM23pUhb_ILN'
```
...
cd tmp_to_transfer
for j in ACetal22_GBM ACetal22_ILN BM23pUhb_GBM BM23pUhb_ILN
do
for i in `seq 1 $num_chains`
do
...
# Please change directories until you are there
# Now, trasnfer the data from the HPC
rsync -avz --copy-links <uname>@<logdetails>:<path>/brate_mammals/tmp_to_transfer/01_posterior .
rsync -avz --copy-link <uname>@<logdetails>:<path>/brate_mammals/pipelines_MCMCtree .
# Remove blank output files
rm pipelines_MCMCtree/*/*sh.o*
```

> [!NOTE]
> If you want to download the output files we generated, please download `02_mammals_divtimes_posterior.zip` from [our FigShare repository](https://doi.org/10.6084/m9.figshare.32033958) and save directory `pipelines_MCMCtree` inside `00_mammals/02_rate_per_branch/00_MCMCtree/sum_analyses`. Then, decompress, `01_posterior.zip` and save `01_posterior` inside the same directory. Please note that this directory also includes the output files we generate during some of the MCMC diagnostics (e.g., directories `[BM23pUhb|ACetal22]_[GBM|ILN]/[PostKPg|Unconstrained]*` and `mcmc_files_*`). If you want to run the subsequent steps, you can move these output files to another location and only keep the four directories with the 6 independent chains (i.e., `[BM23pUhb|ACetal22]_[GBM|ILN]/[1-6]`).
...
## arg7 --> if arg6 is 'Y', arg7 needs to have a name for the `mcmcf4traces_<name>` that will be
##          generated. If `arg6` is equal to 'N', then please write `N` too
./Combine_MCMC.sh ACetal22_GBM mcmc_files_ACetal22_GBM "1 2 3 4" GBM 20000 Y ACetal22_GBM
./Combine_MCMC.sh ACetal22_ILN mcmc_files_ACetal22_ILN "`seq 1 6`" ILN 20000 Y ACetal22_ILN
./Combine_MCMC.sh BM23pUhb_GBM mcmc_files_BM23pUhb_GBM "1 3 4 5 6" GBM 20000 Y BM23pUhb_GBM
./Combine_MCMC.sh BM23pUhb_ILN mcmc_files_BM23pUhb_ILN "`seq 1 6`" ILN 20000 Y BM23pUhb_ILN
```
...
> [!NOTE]
> When you download `02_mammals_divtimes_prior` from [our FigShare repository](https://doi.org/10.6084/m9.figshare.32033958), you will not find the `mcmcf4traces*_[GBM|ILN]` directories as they are somewhat redundant. If you want to generate them, please make sure you run the code above. Please note that directories `mcmc_files*_[GBM|ILN]` will also be generated, so you should move the ones you downloaded from our FigShare elsewhere so that they do not overwrite (in case you want to compare them).
...
# Run from `00_MCMCtree/sum_analyses/01_posterior`
# Please change directories until you are 
# in the directory aforementioned
name_dat=( 'ACetal22_GBM' 'ACetal22_ILN' 'BM23pUhb_GBM' 'BM23pUhb_ILN' )
num_dat=4
count=-1
for i in `seq 1 $num_dat`
do
...
> [!NOTE]
> Please note that we have kept directory `sum_files_posterior` in this repository (see code snippet below). Nevertheless, if you want to also download the `plots` directory (this is generated and populated during MCMC diagnostics), please download `03_mammals_MCMCdiagnostics.zip` from [our FigShare repository](https://doi.org/10.6084/m9.figshare.32033958). Then, decompress this directory and `plots.zip` and save directory `plots` inside `00_mammals/02_rate_per_branch/01_MCMC_diagnostics`. Without the `plots` directory, the commands below will not work. If you have followed this step above, you do not need to repeat this (the `plots` directory included output files for both diagnostics with samples collected when the target distributions was the posterior and the prior).
...
Once the rategrams have been saved for each dataset, partition, and filtered chain, we can run our in-house `R` script [`Summarise_brates.R`](02_Summary_Rates/scripts/Summarise_brates.R), which will generate one rategram per partition and dataset with the mean estimated branch rates for each partition. The output plots will be saved under directory `02_Summary_Rates/plots/rategrams`, in subdirectories with the dataset name.

## 5. Generating summary tables and plots

If you navigate to directory [figs_table](figs_tables/), you shall find three different scripts that you can use to generate informative summary tables and plots that are ready to use for manuscript submission. Please make sure that you have already generated directory `01_MCMC_diagnostics/sum_files_post/sum_divtimes_tsv` (see sections regarding MCMC diagnostics above) and directories `mcmc_files_*` inside `00_MCMCtree/sum_analyses/0*/mcmc_files*`. In this repository, you shall find the [R scripts](figs_tables/scripts), the [output plots](figs_tables/plots/), and the [output summary tables](figs_tables/tables/). If you want to use our `RData` objects so that you do not need to wait for the scripts to read, parse, and process input files, you can download `04_mammals_timetrees_suppmat.zip` [our FigShare repository](https://doi.org/10.6084/m9.figshare.32033958), decompress it, decompress `out_RData.zip`, and save directory `out_RData` inside directory `figs_tables`.
